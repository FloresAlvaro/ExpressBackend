...
export const getUsers = asyncHandler(async (req: Request, res: Response) => {
  const { status } = req.query as { status?: (typeof UserStatus)[keyof typeof UserStatus] };
  const filterStatus = status || UserStatus.ACTIVE;

  const users = await userService.getAllUsers(filterStatus);
  logger.info({ count: users.length, filter: filterStatus }, 'Usuarios obtenidos exitosamente');
...
export const restoreUserController = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!restoredUser) {
    logger.warn({ userId: id }, 'Intento de restaurar usuario inexistente o ya activo');
    throw new NotFoundError('Usuario');
  }
